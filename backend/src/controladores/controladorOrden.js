...
exports.crearOrdenDesdeCarrito = async (req, res) => {
  ...
  try {
    ...
    if (isNaN(usuarioId)) {
      return res
        .status(400)
        .json({ mensaje: "El usuarioId debe ser un número válido" });
    }
    ...
    if (usuarioResult.rowCount === 0) {
      return res
        .status(404)
        .json({ mensaje: "Usuario no encontrado en PostgreSQL" });
    }
    ...
    if (!carrito) {
      return res.status(404).json({
        mensaje: "Carrito no encontrado para el usuario especificado",
      });
    }
    ...
    res
      .status(201)
      .json({ mensaje: "Orden creada con éxito", orden: nuevaOrden });
  } catch (error) {
    res
      .status(500)
      .json({ mensaje: "Error al crear la orden", error: error.message });
  }
};
...
exports.obtenerOrden = async (req, res) => {
  ...
  try {
    ...
    res
      .status(500)
      .json({ mensaje: "Error al obtener la orden", error: error.message });
  }
};
