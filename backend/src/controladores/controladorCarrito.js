...
exports.crearCarrito = async (req, res) => {
  ...
  try {
    ...
    if (productosValidos.length !== productosIds.length) {
      return res
        .status(400)
        .json({ mensaje: "Uno o más productos no son válidos" });
    }
    ...
    if (carritoExistente) {
      return res
        .status(400)
        .json({ mensaje: "Ya existe un carrito para este usuario" });
    }
    ...
    res
      .status(500)
      .json({ mensaje: "Error al crear el carrito", error: error.message });
  }
};
...
exports.eliminarCarrito = async (req, res) => {
  ...
  try {
    ...
    res
      .status(500)
      .json({ mensaje: "Error al eliminar el carrito", error: error.message });
  }
};
